...
router.get('/setup', (ctx) => ctx.response.with(Kit.Setup.GetHandler(Resp.HTML) as Response))
router.post('/setup', (ctx) => ctx.response.with(Kit.Setup.PostHandler(Resp.HTML) as Response))
router.options('/setup', (ctx) => ctx.response.with(Kit.Setup.OptionsHandler(Resp.HTML) as Response))
router.get(
  '/.well-known/webfinger',
  (ctx) => ctx.response.with(Kit.WebFinger.GetHandler(ctx.request.url, testUsers, Resp.WebFinger, config) as Response),
)
router.options('/.well-known/webfinger', (ctx) => ctx.response.with(Resp.WebFinger.options204() as Response))
router.get(
  '/.well-known/nodeinfo',
  (ctx) => ctx.response.with(Kit.NodeInfo.GetHandler(Resp.NodeInfo, config) as Response),
)
router.options('/.well-known/nodeinfo', (ctx) => ctx.response.with(Resp.NodeInfo.options204() as Response))
router.get('/nodeinfo/2.1', (ctx) => ctx.response.with(Kit.NodeInfo.GetHandler21(Resp.NodeInfo, config) as Response))
router.options('/nodeinfo/2.1', (ctx) => ctx.response.with(Resp.NodeInfo.options204() as Response))
//router.get('/activitypub/server', (ctx) => ctx.response.with(Kit.ServerActor.GetHandler(config, null, Resp.ActivityPub) as Response))
router.options('/activitypub/server', (ctx) => ctx.response.with(Resp.ActivityPub.options204() as Response))
// router.get('/activitypub/server/inbox', (ctx) => ctx.response.with(Kit.ServerActor.inboxGetHandler(Resp.ActivityPub) as Response))
router.options('/activitypub/server/inbox', (ctx) => ctx.response.with(Resp.ActivityPub.options204() as Response))
// router.get('/activitypub/server/outbox', (ctx) => ctx.response.with(Kit.ServerActor.outboxGetHandler(Resp.ActivityPub) as Response))
