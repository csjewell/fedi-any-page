/* SPDX-License-Identifier: MIT
 * SPDX-FileCopyrightText: 2025 Curtis Jewell and other contributors
 */
//@ ts-check
import { readFileSync, writeFileSync } from 'node:fs'
import { cwd } from 'node:process'
import { format } from 'node:util'
import { ReflectionKind } from 'typedoc'
import { MarkdownPageEvent } from 'typedoc-plugin-markdown'

/**
 * @param {import('typedoc-plugin-markdown').MarkdownApplication} app
 * @return {void}
 */
export const load = (app) => {
  let weight = 100

  app.renderer.on(
    MarkdownPageEvent.BEGIN,
    /**
     * @param {import('typedoc-plugin-markdown').MarkdownPageEvent} page
     * @return {void}
     */
    (page) => {
      /**
       * Update page.frontmatter object using information from the page model
       */
      switch (page.model.kind) {
        case ReflectionKind.Module: {
          if (page.model.packageVersion === undefined) {
          // This is the "-internal-.md" file.
            page.frontmatter = {
              title : format('%s - %s  - internal declarations', page.model.parent.name, page.model.parent.packageVersion),
              weight,
              ...page.frontmatter,
            }
          } else {
            page.frontmatter = {
              title : format('%s - %s', page.model.name, page.model.packageVersion),
              weight,
              ...page.frontmatter,
            }
          }

          break
        }
        case ReflectionKind.Namespace: {
          let title = page.model.name
          let model = page.model.parent

          while (model.kind ===    ReflectionKind.Namespace) {
            title = format('%s - %s', model.name, title)
            model = model.kind
          }
          page.frontmatter = {
            title : format('%s - %s - %s', page.model.name, page.model.packageVersion, title),
            weight,
            ...page.frontmatter,
          }

          break
        }
        case ReflectionKind.Project: {
          page.frontmatter = {
            title : '@csjewell-activitypub/',
            weight,
            draft : true,
            ...page.frontmatter,
          }

          break
        }
        default:
      // Do nothing
      }
      weight += 1
    },
  )
  app.renderer.postRenderAsyncJobs.push(
    /**
     * @param {import('typedoc-plugin-markdown').MarkdownRendererEvent} renderer
     * @return {void}
     */
    (renderer) => {
      // The navigation JSON structure is available on the navigation object.
      const {navigation,} = renderer
      const slugs = {
        'localserver-h3'         : 'latest/localserver-h3',
        'localserver-hapi'       : 'latest/localserver-hapi',
        'eslint-config'          : 'latest/eslint-config',
        're-pliers'              : 'latest/re-pliers',
        'database-node-sqlite'   : 'latest/database-node-sqlite',
        'database-better-sqlite' : 'latest/database-better-sqlite/start',
        'database-cloudflare-d1' : 'latest/database-cloudflare-d1/start',
        'database-mock'          : 'latest/database-mock/start',
        'general'                : 'latest/general/start',
        'handlers-response'      : 'latest/handlers-response/start',
        'types'                  : 'latest/types/start',
      }
      const pages = []

      navigation.forEach((value) => {
        const slugString = value.title.slice(Math.max(0, value.title.lastIndexOf('/') + 1))

        pages.push({
          title : value.title,
          slug  : slugs[ slugString ],
        })
      })

      const out = JSON.parse(readFileSync(`${ cwd() }/../fedi-any-page-docs/data/sidebar.json`))

      if (!Array.isArray(out)) {
        throw new TypeError()
      }

      out.push({ title: 'Module Documentation', pages, })
      writeFileSync(`${ cwd() }/../fedi-any-page-docs/data/navigation.json`, JSON.stringify(out))
    },
  )
}
